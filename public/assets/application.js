<<<<<<< HEAD:tmp/cache/assets/D84/210/sprockets%2Fabd0103ccec2b428ac62c94e4c40b384
o: ActiveSupport::Cache::Entry	:@compressedF:@expires_in0:@created_atf1362631882.7593997:@value{	I"length:EFiÎSI"digest;
F"%709c5d3258bf884b94434d265cba517eI"source;
FI"ÎS/*!
=======
/*!
>>>>>>> 62af39c8eb88af0831345437ee8c844f5d657a50:public/assets/application.js
...
(function() {
  ...
  $(document).ready(function() {
    var fetch_item, searchPress, showInTable, showResults;
    fetch_item = function(page, per) {
      ...
      return $.ajax({
        ...
        beforeSend: function() {
          ...
          return $('.stdtable').prepend('<img src="/assets/loading_128.gif" style="position: absolute; left: ' + (width / 2 - 64) + 'px; top: 128px" class="img-loading"/>');
        },
        success: function(response) {
          $('.stdtable .img-loading').remove();
          $('.stdtable > tbody').empty();
          $('.stdtable > tbody').css('opacity', '1');
          return showResults(response, page, per);
        }
      });
    };
    showResults = function(response, page, per) {
      var i, num, _i;
      if (per !== void 0) {
      ...
      if (response.data.length > 0) {
        return showInTable(response.data);
      } else {
        return $('.stdtable > tbody').append('<tr><td colspan="5" style="text-align: center">Not Found Results</td></tr>');
      }
    };
    showInTable = function(data) {
      if (data === void 0 || data === null || data.length === 0) {
        return;
      }
      return $.each(data, function(index, value) {
        return $('.stdtable > tbody').append('<tr id="proid-' + value['id'] + '"><td class="center"><span class="checkbox"><input type="checkbox"></span></td><td class="center">' + value['id'] + '</td><td class="center">' + value['name'] + '</td><td class="center">' + value['description'] + '</td><td class="center">' + value['server_id'] + '</td></tr>');
      });
    };
    searchPress = function(content, type) {
      if (type === void 0) {
        ...
        success: function(response) {
          ...
          if (response.data.length === 0) {
            if (type === 'promotion_id') {
              ...
              $('.stdtable > tbody').empty();
              $('.stdtable > tbody').css('opacity', '1');
              return $('.stdtable > tbody').append('<tr><td colspan="6" style="text-align: center">Not Found Results <a href="/product_promotions/new?promotion_id=' + content + '">Want to Create?</a></td></tr>');
            } else {
              return showResults(response, 1, response.per);
            }
          } else {
            return showResults(response, 1, response.per);
          }
    ...
    $('#lists-item .btn-search').click(function() {
      var value;
      value = $(this).parent().find('.search-content').children('option[selected="selected"]').attr('value');
      return searchPress($(this).parent().children('input').val());
    });
    $('#lists-item #promotions-per-page').change(function() {
      var per;
      per = $(this).val();
      return fetch_item(1, per);
    });
    $('#lists-item #btn-delete button').click(function() {
      var list;
      list = [];
      $.each($('.stdtable > tbody').find('span.checkbox input[type=checkbox][checked=checked]'), function(index, value) {
        var tr;
        tr = $(value).closest('tr');
        return list.push(tr.attr('id').substr(6));
      });
      if (list.length > 0) {
        return jConfirm(I18n.t('common.dialog.confirm.content'), I18n.t('common.dialog.confirm.title'), function(r) {
          var page_active;
          if (r === true) {
            $.each($('.stdtable > tbody').find('span.checkbox input[type=checkbox][checked=checked]'), function(index, value) {
              var tr;
            ...
            return $.ajax({
              ...
              data: {
                list: list,
                page: page_active,
                content: $('#dyntable_filter > input').val(),
                type: $('#dyntable_filter .search-content').val()
              },
              success: function(response) {
    ...
    $('#lists-item #promo-search').keyup(function(e) {
      var content;
      content = $(this).val();
      if (e.which === 13) {
        return searchPress(content);
      } else {
        return;
        return $.each($('.stdtable > tbody > tr'), function(index, value) {
    ...
    $('#lists-item .paginate_button').paginate({
      callback: fetch_item
    });
    $('#dyntable_filter .search-content').change(function() {
      var value;
      ...
      if (value === 'user_entered' || value === 'imported_only') {
        ...
        $(this).closest('.dataTables_filter').children('.search-select').css({
          'position': 'static',
          'margin-right': '28px'
        });
      } else {
      ...
      return $('#dyntable_filter > input').css('padding-left', ($('.search-select').outerWidth(true) + 4) + 'px');
    });
    $('#dyntable_length button').click(function() {
      var obj, type;
      type = $(this).attr('type');
      if (type === 'new') {
    ...
    return $('body > .popup .footer button').click(function() {
      var description, name, obj, tag_popup, type;
      type = $(this).attr('type');
      if (type === 'cancel') {
        return $(this).closest('.popup').addClass('hide');
      } else {
        obj = $(this);
...
var I18n = I18n || {};
I18n.translations = {"en":{"date":{"formats":{"default":"%Y-%m-%d","short":"%b %d","long":"%B %d, %Y"},"day_names":["Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"],"abbr_day_names":["Sun","Mon","Tue","Wed","Thu","Fri","Sat"],"month_names":[null,"January","February","March","April","May","June","July","August","September","October","November","December"],"abbr_month_names":[null,"Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"],"order":["year","month","day"]},"time":{"formats":{"default":"%a, %d %b %Y %H:%M:%S %z","short":"%d %b %H:%M","long":"%B %d, %Y %H:%M"},"am":"am","pm":"pm"},"support":{"array":{"words_connector":", ","two_words_connector":" and ","last_word_connector":", and "}},"errors":{"format":"%{attribute} %{message}","messages":{"inclusion":"is not included in the list","exclusion":"is reserved","invalid":"is invalid","confirmation":"doesn't match confirmation","accepted":"must be accepted","empty":"can't be empty","blank":"can't be blank","too_long":"is too long (maximum is %{count} characters)","too_short":"is too short (minimum is %{count} characters)","wrong_length":"is the wrong length (should be %{count} characters)","not_a_number":"is not a number","not_an_integer":"must be an integer","greater_than":"must be greater than %{count}","greater_than_or_equal_to":"must be greater than or equal to %{count}","equal_to":"must be equal to %{count}","less_than":"must be less than %{count}","less_than_or_equal_to":"must be less than or equal to %{count}","odd":"must be odd","even":"must be even"}},"activerecord":{"errors":{"messages":{"taken":"has already been taken","record_invalid":"Validation failed: %{errors}"}}},"number":{"format":{"separator":".","delimiter":",","precision":3,"significant":false,"strip_insignificant_zeros":false},"currency":{"format":{"format":"%u%n","unit":"$","separator":".","delimiter":",","precision":2,"significant":false,"strip_insignificant_zeros":false}},"percentage":{"format":{"delimiter":""}},"precision":{"format":{"delimiter":""}},"human":{"format":{"delimiter":"","precision":3,"significant":true,"strip_insignificant_zeros":true},"storage_units":{"format":"%n %u","units":{"byte":{"one":"Byte","other":"Bytes"},"kb":"KB","mb":"MB","gb":"GB","tb":"TB"}},"decimal_units":{"format":"%n %u","units":{"unit":"","thousand":"Thousand","million":"Million","billion":"Billion","trillion":"Trillion","quadrillion":"Quadrillion"}}}},"datetime":{"distance_in_words":{"half_a_minute":"half a minute","less_than_x_seconds":{"one":"less than 1 second","other":"less than %{count} seconds"},"x_seconds":{"one":"1 second","other":"%{count} seconds"},"less_than_x_minutes":{"one":"less than a minute","other":"less than %{count} minutes"},"x_minutes":{"one":"1 minute","other":"%{count} minutes"},"about_x_hours":{"one":"about 1 hour","other":"about %{count} hours"},"x_days":{"one":"1 day","other":"%{count} days"},"about_x_months":{"one":"about 1 month","other":"about %{count} months"},"x_months":{"one":"1 month","other":"%{count} months"},"about_x_years":{"one":"about 1 year","other":"about %{count} years"},"over_x_years":{"one":"over 1 year","other":"over %{count} years"},"almost_x_years":{"one":"almost 1 year","other":"almost %{count} years"}},"prompts":{"year":"Year","month":"Month","day":"Day","hour":"Hour","minute":"Minute","second":"Seconds"}},"helpers":{"select":{"prompt":"Please select"},"submit":{"create":"Create %{model}","update":"Update %{model}","submit":"Save %{model}"}},"will_paginate":{"previous_label":"&#8592; Previous","next_label":"Next &#8594;","page_gap":"&hellip;","page_entries_info":{"single_page":{"zero":"No %{model} found","one":"Displaying 1 %{model}","other":"Displaying all %{count} %{model}"},"single_page_html":{"zero":"No %{model} found","one":"Displaying <b>1</b> %{model}","other":"Displaying <b>all&nbsp;%{count}</b> %{model}"},"multi_page":"Displaying %{model} %{from} - %{to} of %{count} in total","multi_page_html":"Displaying %{model} <b>%{from}&nbsp;-&nbsp;%{to}</b> of <b>%{count}</b> in total"}},"common":{"save":"Save","edit":"Edit","add":"Add","new":"New","delete":"Delete","next":"Next","first":"First","previous":"Previous","last":"Last","type_something":"type something...","generate":"Generate","update":"Update","quote":"Quote","insert":"Insert","dialog":{"confirm":{"title":"Confirmation Dialog","content":"Are you sure about that?"}},"created_at":"Created at","last_modified":"Last Modified","want_to_create":"Want to create? ","no_result":"Not Found Results"},"views":{"home_folder":{"index_view":{"title":"Restful API"}}},"active_records":{"client_model":{"id":"ID","name":"Name","description":"Description","server_id":"From Server"}}}};
...
;
TI"_version;
F"%d0c5befff3f5bc9e8415b3f4cd158828
=======
>>>>>>> 62af39c8eb88af0831345437ee8c844f5d657a50:public/assets/application.js
